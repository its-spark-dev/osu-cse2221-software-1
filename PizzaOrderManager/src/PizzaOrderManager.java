...
public final class PizzaOrderManager {
    ...
    private static void getPriceMap(String fileName, Map<String, Integer> priceMap) {
        ...
        /*
         * Clear the map to replace it with new entries
         */
        priceMap.clear();

        while (!input.atEOS()) {
            /*
             * Read the entire line and split by comma
             */
            String line = input.nextLine();
            int commaIndex = line.indexOf(',');
            String item = line.substring(0, commaIndex).trim();
            int price = Integer.parseInt(line.substring(commaIndex + 1).trim());

            /*
             * Add the item and its price to the map
             */
    ...
    private static int getOneOrder(SimpleReader input, Map<String, Integer> sizePriceMap,
        ...
        int totalPrice = sizePriceMap.value(size);
        ...
        while (!input.atEOS()) {
            ...
            totalPrice += toppingPriceMap.value(line);
        }
